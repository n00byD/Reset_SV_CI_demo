...
if defined LISA_HOME if exist "%LISA_HOME%\jre" (
...
if [%WEB%] == [true] (
   echo Starting ForwardCars - WEB
      start /min "ForwardCars - WEB" java -Dfile.encoding=UTF8 %WEB_AGENT% -jar cars-app-10.3.0.war
      java -cp cars-admin-10.3.0.jar WaitForAppReady http://localhost:3434/cars-app/health 100
)

if [%ESB%] == [true] (
   echo Starting ForwardCars - ESB
      start /min "ForwardCars - ESB" java -Dfile.encoding=UTF8 %ESB_AGENT% -jar cars-esb-10.3.0.war
       java -cp cars-admin-10.3.0.jar WaitForAppReady http://localhost:3700/cars-esb/health 100
)

if [%INV%] == [true] (
   echo Starting ForwardCars - INVENTORY
      start /min "ForwardCars - INVENTORY" java -Dfile.encoding=UTF8 %INV_AGENT% -jar cars-inventory-10.3.0.war
      java -cp cars-admin-10.3.0.jar WaitForAppReady http://localhost:3500/cars-inventory/health 100
)

if [%LOAN%] == [true] (
   echo Starting ForwardCars - LOAN
      start /min "ForwardCars - LOAN" java -Dfile.encoding=UTF8 %LOAN_AGENT% -jar cars-loan-10.3.0.war
      java -cp cars-admin-10.3.0.jar WaitForAppReady http://localhost:3600/cars-loan/health 100
...
if [%WEB%] == [true] (
   ...
   java -cp cars-admin-10.3.0.jar ShutdownApp http://localhost:3434/cars-app/shutdown
)

if [%ESB%] == [true] (
   echo Stopping ForwardCars - ESB
   java -cp cars-admin-10.3.0.jar ShutdownApp http://localhost:3700/cars-esb/shutdown
)

if [%INV%] == [true] (
   echo Stopping ForwardCars - INVENTORY
   java -cp cars-admin-10.3.0.jar ShutdownApp http://localhost:3500/cars-inventory/shutdown
)

if [%LOAN%] == [true] (
   echo Stopping ForwardCars - LOAN
   java -cp cars-admin-10.3.0.jar ShutdownApp http://localhost:3600/cars-loan/shutdown
)
